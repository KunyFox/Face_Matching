...
def build_classfier(num_layers, num_classes, pretrained_backone=None, resume=None):
    backbone = get_resnet(num_layers)
    if resume:
        net = ClsLogits(backbone, num_classes, resume)
    elif pretrained_backone:
        backbone.load_pretrained_parameters(pretrained_backone)
        net = ClsLogits(backbone, num_classes)
    else:
        net = ClsLogits(backbone, num_classes)
...
def main():
    ...
    net.cast(opt.dtype)
    # net.load_parameters('renet50_v1_8k.params', ctx = context)
    if opt.resume_params != '':
        logger.info('Resuming from {}'.format(opt.resume_params))
    ...
    def train(ctx):
        ...
        best_val_score = 1
        logger.info('Start trining with total {} epochs'.format(opt.num_epochs - opt.resume_epoch))
        for epoch in range(opt.resume_epoch, opt.num_epochs):
            ...
            for i, batch in enumerate(train_data):
                ...
                if opt.log_interval and not (i+1)%opt.log_interval:
                    ...
                    loss = [l.asnumpy() for l in loss]
                    loss = sum(loss)[0] / opt.num_gpus
                    logger.info('Epoch[%d] Batch [%d]\tSpeed: %f samples/sec\tLoss=%.5f\tlr=%f'%(
